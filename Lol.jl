...
# Verifie si le noeud place en parametre est un noeud isole, renvoie true si oui false sinon
function isIsolated(i,n,mat)
  isIsolated=true
  for k=1:n
    if (mat[i,k]!=Inf)
    isIsolated=false
...
function floydWarshall(i,j,matcost)
  if (i==j) # distance = 0, depart = arrivee
    return [0.0,[i]]
  end
   n=size(matcost,1)
...
function main()
  ...
  while(!check)
    ...
    if ((node1<1 || node1>size(matInput,1)) || (node2<1 || node2>size(matInput,1)))
      println("At least one of your entry node doesn't exist! Please choose correct nodes!")
    else
      check=true
    end
  end
  ...
  # cas particulier ou le noeud d'arrivee ou le noeud d'entree est un noeud isole
  if (isIsolated(node1,size(matInput,1),matcount) || isIsolated(node2,size(matInput,1),matcount))
    println("One of your entry node is a isolated node so the distance is infinite")
  else
